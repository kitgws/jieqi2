...
case "update":
	...
	if (empty($errtext)) {
		...
		if ($article->getVar("articlecode", "n") != $_POST["articlecode"]) {
			if (0 < $article_handler->getCount(new Criteria("articlecode", $_POST["articlecode"], "="))) {
				if ($customacode) {
					...
					$$_POST["articlecode"] .= "_" . $article->getVar("articleid", "n");
				}
			}
		}
		...
		if ($ismanager) {
			...
			if (isset($_POST["master"]) && (0 < strlen($_POST["master"]))) {
				include_once (JIEQI_ROOT_PATH . "/class/users.php");
				$users_handler = &JieqiUsersHandler::getInstance("JieqiUsersHandler");
				$masterobj = $users_handler->getByname($_POST["master"], 3);
			...
			if (isset($_POST["foreword"])) {
				$article->setVar("foreword", $_POST["foreword"]);
			}

			if (isset($_POST["siteid"]) && is_numeric($_POST["siteid"]) && !empty($jieqiSites[$_POST["siteid"]]["custom"])) {
				$article->setVar("siteid", intval($_POST["siteid"]));
			}

			if (isset($_POST["monthly"]) && is_numeric($_POST["monthly"]) && isset($jieqiOption["article"]["monthly"]["items"][$_POST["monthly"]])) {
				$article->setVar("monthly", intval($_POST["monthly"]));
			}
			...
			if (empty($rgroup) && isset($jieqiOption["article"]["rgroup"]["items"][$_POST["rgroup"]])) {
		...
		if (!$article_handler->insert($article)) {
			...
			if (0 < $vipid) {
				...
				$sql = "UPDATE " . jieqi_dbprefix("obook_obook") . " SET siteid = " . intval($article->getVar("siteid", "n")) . ", obookname = '" . jieqi_dbslashes($article->getVar("articlename", "n")) . "', keywords = '" . jieqi_dbslashes($article->getVar("keywords", "n")) . "', initial = '" . jieqi_dbslashes($article->getVar("initial", "n")) . "', sortid = " . intval($article->getVar("sortid", "n")) . ", intro = '" . jieqi_dbslashes($article->getVar("intro", "n")) . "', notice = '" . jieqi_dbslashes($article->getVar("notice", "n")) . "', authorid = " . intval($article->getVar("authorid", "n")) . ", author = '" . jieqi_dbslashes($article->getVar("author", "n")) . "', agentid = " . intval($article->getVar("agentid", "n")) . ", agent = '" . jieqi_dbslashes($article->getVar("agent", "n")) . "', masterid = " . intval($article->getVar("masterid", "n")) . ", master = '" . jieqi_dbslashes($article->getVar("master", "n")) . "', posterid = " . intval($article->getVar("posterid", "n")) . ", poster = '" . jieqi_dbslashes($article->getVar("poster", "n")) . "', publishid = " . intval($article->getVar("vippubid", "n")) . ", imgflag = " . intval($article->getVar("imgflag", "n")) . ", canbesp = " . intval($article->getVar("monthly", "n")) . " WHERE obookid = " . $vipid;
				$query->execute($sql);
			...
			if (!empty($_FILES["articlelpic"]["name"]) && !empty($_FILES["articlespic"]["name"])) {
				...
				jieqi_article_coverdo($_FILES["articlelpic"], "l");
				if (!empty($_POST["sameslpic"]) || empty($old_simg)) {
					jieqi_copyfile($_FILES["articlelpic"]["tmp_name"], $package->getDir("imagedir") . "/" . $id . "l" . $limage_postfix, 511, false);
					jieqi_article_coverdo($_FILES["articlelpic"], "s");
					jieqi_copyfile($_FILES["articlelpic"]["tmp_name"], $package->getDir("imagedir") . "/" . $id . "s" . $limage_postfix, 511, true);
				}
				else {
					jieqi_copyfile($_FILES["articlelpic"]["tmp_name"], $package->getDir("imagedir") . "/" . $id . "l" . $limage_postfix, 511, true);
				}
...
default:
	...
	if (2 <= floatval(JIEQI_VERSION)) {
		...
		$jieqiTpl->assign_by_ref("taglimit", $taglimit);
	}
